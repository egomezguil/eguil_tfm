...
params <- list(
  Sup="Sup",
  Nod="Nod",
  directorio= "/Users/estefaniagomezguil/Desktop/SubtiposRecod"
)
ruta_sup<- file.path(params$directorio, "Superficial")
ruta_nod<- file.path(params$directorio, "Nodular")

files_sup <- list.files(path = ruta_sup, pattern = ".txt", full.names = TRUE)
files_nod <- list.files(path = ruta_nod, pattern = ".txt", full.names = TRUE)
Grupo <- c (rep(params$Sup, length(files_sup)), rep(params$Nod, length(files_nod)))
files <- c(files_sup, files_nod)
print(files)
...
colnames(design) = c(params$Sup, params$Nod)
design
fit = lmFit(yfilt, design)
```

```{r}
parametre1 = params$Sup
parametre2 = params$Nod

contraste = makeContrasts(Sup-Nod, levels = design)
