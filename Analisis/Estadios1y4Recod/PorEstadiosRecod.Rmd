---
title: "Análisis estadios con datos recodificados"
...
params <- list(
  estadio1="Est1",
  estadio4="Est4",
  directorio= "/Users/estefaniagomezguil/Desktop/Estadios Recod"
)
ruta_est1<- file.path(params$directorio, "1")
ruta_est4<- file.path(params$directorio, "4")

files_est1 <- list.files(path = ruta_est1, pattern = ".txt", full.names = TRUE)
files_est4 <- list.files(path = ruta_est4, pattern = ".txt", full.names = TRUE)
Grupo <- c (rep(params$estadio1, length(files_est1)), rep(params$estadio4, length(files_est4)))
files <- c(files_est1, files_est4)
print(files)
...
colnames(design) = c(params$estadio1, params$estadio4)
design
fit = lmFit(yfilt, design)
```

```{r}
parametre1 = params$estadio1
parametre2 = params$estadio4

contraste = makeContrasts(Est1-Est4, levels = design)
